...
class KafkaController:
    def __init__(self) -> None:
        self.admin = KafkaAdminClient(bootstrap_servers=[f'{settings.kafka_host}:{settings.kafka_port}'])
        self.producer = KafkaProducer(bootstrap_servers=[f'{settings.kafka_host}:{settings.kafka_port}'])
